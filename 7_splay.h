...
Node<T>* Node<T>::remove(T val) {
	...
	Node<T>* next = found -> right;
	while(next -> left != 0) {
		next = next -> left;
	}

	found -> value = next -> value;
	return next -> remove(next -> value);
}
...
void SplayTree<T>::remove(T val) {
	if (root != 0) {
		if (val == root->value) {
			...
			Node<T> *parentNode = root -> remove(val);
			root = root -> splay(root,parentNode);
		}
	} 
}
